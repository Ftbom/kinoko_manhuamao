     ...
     */
     async load(state) {
        try {
            ...
            let data = [];
            let script = doc.querySelectorAll('script:not([str])')[11].textContent;
            let ctx = new ScriptContext();
            ctx.eval(script); //将字符串做代码运行，获取img_data
            ctx.eval(LZString); //解密
            let img_urls = await ctx.eval('LZString.decompressFromBase64(img_data)');
            for (let img_url of img_urls.split(',')) {
                data.push({
                    url: baseURL + "/uploads/" + img_url,
                    headers: {
                        Referer: url
                    }
                });
